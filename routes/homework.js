...
module.exports = (pool, DOMPurifyInstance) => {
    ...
    // 处理作业提交
    router.post('/setc', upload.array('images', 10), async (req, res) => {
        try {
            ...
            if (images && images.length > 0) {
                imagePaths = images.map(file => `data:image/png;base64,${file.buffer.toString('base64')}`);
            }
            ...
            res.status(500).send('服务器错误，作业提交失败');
        }
    });
